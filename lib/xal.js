...
function setUrl(ip, port) {
    agent.url = 'http://' + ip + ':' + port;
}
...
function generateEventList(intialState, updatedState) {
    ...
    for (var i = 0; i < eventListeners.length; i++) {
        if (!_.isEqual(
                getValue(intialState, eventListeners[i].event),
        ...
        for (var i = 0; i < keys.length; ++i) {
            if (!currObj.hasOwnProperty(keys[i])) {
                if (i === keys.length - 1) {
                    ...
                    log.info({currObj: currObj[keys[i]]});
                } else {
                    currObj[keys[i]] = {};
                }
...
function runEventHandlers(eventId) {
    ...
    var runEventListener = function(cb) {
        for (var j = 0; j < eventList.length; j++) {
            eventList[i].callback(newState, cb);
        }
    };
