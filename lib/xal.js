...
function updateEvent(eventId, xiKey, obj, cb) {
    ...
    putEvent(eventHash[eventId].internalState, cb);
}

function ensureConsistentState(state) {
    var eventId = state.get('xi.event.id');
    for (var xiKey in eventHash[eventId].putValues) {
        if (eventHash[eventId].putValues.hasOwnProperty(xiKey)) {
            state.put(xiKey, eventHash[eventId].putValues[xiKey]);
        }
    }
}
...
function augmentStateWithMethods(state) {
    ...
    state.put = function(xiKey, data) {
        var eventId = state.get('xi.event.id');
        eventHash[eventId].putValues[xiKey] = data;
        var tuple;
        ...
        if (eventList.length === i) {
            ...
            if (eventHash[eventId].newState) {
                ...
                if(!_.isEqual(eventHash[eventId].sentState , eventHash[eventId].internalState)){
                    ...
                    xal.log.info("State did not change. Not sending");
                }
...
function event(req, res, next) {
    ...
    ensureConsistentState(eventHash[eventId].newState);
